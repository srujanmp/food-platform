...
func JWTAuth(jwtSecret string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if authHeader == "" || !strings.HasPrefix(authHeader, "Bearer ") {
			c.AbortWithStatusJSON(http.StatusUnauthorized, ErrorResponse{Error: "missing_or_invalid_authorization_header"})
			return
		}
		...
		if err != nil || !token.Valid {
			c.AbortWithStatusJSON(http.StatusUnauthorized, ErrorResponse{Error: "invalid_or_expired_token"})
			return
		}
		claims, ok := token.Claims.(*jwt.MapClaims)
		if !ok {
			c.AbortWithStatusJSON(http.StatusUnauthorized, ErrorResponse{Error: "invalid_token_claims"})
			return
		}
...
func RequireRole(roles ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.AbortWithStatusJSON(http.StatusForbidden, ErrorResponse{Error: "insufficient_permissions"})
	}
}
